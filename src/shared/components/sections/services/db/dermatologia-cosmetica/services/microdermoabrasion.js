/* eslint max-len: [2, 1000, 4] */

module.exports = [{
  type: 'title',
  text: 'Microdermoabrasión',
  className: '',
}, {
  type: 'image',
  src: '/images/servicios/dermatologia-cosmetica/14_microdermoabracion.jpg',
  text: 'Tratamiento 360',
}, {
  type: 'subtitle',
  className: '',
  text: '¿Qué es?',
}, {
  type: 'text',
  className: 'normal_break',
  text: [
    'Una piel de mejor calidad, suave y limpia es el resultado de la microdermoabrasión, la cual ayuda a eliminar las capas exteriores de la piel dejándola libre de células muertas, ayudando a que las células se regeneren para dar paso a una piel nueva.',
    'La microdermoabrasión es una forma de exfoliación mecánica que ayuda también a la producción de colágeno en la piel.',
  ],
}, {
  type: 'subtitle',
  className: '',
  text: 'Objetivos',
}, {
  type: 'list',
  className: '',
  items: [
    'Eliminar impurezas e imperfecciones.',
    'Eliminar manchas de sol.',
    'Reduce poros dilatados.',
    'Aminorar arrugas.',
    'Suavizar cicatrices.',
  ],
}, {
  type: 'subtitle',
  className: '',
  text: '¿Cómo funciona?',
}, {
  type: 'text',
  className: 'normal_break',
  text: [
    'Con el afán de cuidar tu piel, en lasermedica realizamos de 5 a 10 sesiones dependiendo de la condición de la piel y la profundidad del daño que tenga, cada sesión dura alrededor de media hora en la que se realiza una exfoliación a profundidad con lo que se eliminan células muertas logrando regeneración celular y producción de colágeno.',
    'La microdermoabrasión es un tratamiento muy rápido y efectivo, que no duele y te permite regresar a tus actividades diarias sin ninguna complicación; existen recomendaciones para optimizar los resultados de la microdermoabrasión, como evitar la exposición solar, utilizar bloqueador y otros consejos que nuestros profesionales te invitarán a seguir para que tu piel luzca radiante.',
  ],
}, {
  type: 'link',
  className: 'btn7red',
  text: 'AGENDAR CITA',
  href: '/contacto/microdermoabrasion',
}];
