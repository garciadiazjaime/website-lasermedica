...
var Services = function (_React$Component) {
  ...
  _createClass(Services, [{
    ...
    value: function renderServices(data) {
      var renderServiceChildren = function renderServiceChildren(items) {
        var className = items.length > 5 ? 'col-xs-12 col-md-6' : 'col-sm-12';
        return items.map(function (service, index) {
          return _react2.default.createElement(
            'div',
            { className: className + ' ' + style.serviceElement },
            _react2.default.createElement(
              _reactRouter.Link,
              { to: service.href, title: service.title, key: index },
              service.title
            )
          );
        });
      ...
      var servicesEl = data.map(function (item, index) {
        var childrenEl = renderServiceChildren(item.items);
        return _react2.default.createElement(
          'div',
          { className: 'col-sm-4' },
          _react2.default.createElement(
            _reactRouter.Link,
            { className: style.serviceTitle, to: item.href, title: item.title, key: index },
            item.title
          ),
          _react2.default.createElement(
            'div',
